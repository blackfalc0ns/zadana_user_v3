...
class _VendorComparisonSheetState extends State<_VendorComparisonSheet> {
  ...
  List<MapEntry<CartVendorEntity, double>> get _sortedResults {
    return _selectedIds
        .map(
          (id) => MapEntry(
            widget.vendors.firstWhere((v) => v.id == id),
            _totalFor(id),
          ),
        )
        .toList()
  ...
  Widget _buildVendorResultCard(
    CartVendorEntity vendor,
    double total,
    int rank,
    bool isFirst,
  ) {
                ...
                Container(
                  ...
                  child: Center(
                    child: Icon(
                      Icons.storefront_rounded,
                      size: 18,
                      color: isFirst ? AppColors.white : AppColors.white,
                    ),
                  ),
                ),
                const SizedBox(width: 12),
