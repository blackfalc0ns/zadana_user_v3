...
class DeliveryOtpViewModel extends Cubit<DeliveryOtpState> {
  ...
  Future<void> _sendOtp(String orderId, String phoneNumber) async {
    ...
    switch (result) {
      case ApiSuccessResult():
        developer.log('OTP sent successfully', name: 'DeliveryOtpViewModel');
        _startResendTimer();
        emit(state.copyWith(isLoading: false, canResend: false));
      case ApiErrorResult():
        developer.log(
        ...
        emit(
          state.copyWith(isLoading: false, errorMessage: result.failure.code),
        );
    }
  }
  ...
  Future<void> _verifyOtp(String orderId, String otpCode) async {
    if (state.remainingAttempts <= 0) {
      emit(state.copyWith(errorMessage: 'delivery_otp_attempts_exceeded'));
      return;
    }
  ...
  Future<void> _resendOtp(String orderId) async {
    ...
    switch (result) {
      case ApiSuccessResult():
        developer.log('OTP resent successfully', name: 'DeliveryOtpViewModel');
        _startResendTimer();
        emit(state.copyWith(isLoading: false, canResend: false));
      case ApiErrorResult():
        developer.log(
  ...
  void _startResendTimer() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      if (timerValue <= 0) {
        timer.cancel();
        emit(state.copyWith(resendTimer: 30, canResend: true));
      } else {
